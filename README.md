...
The helper script is available at: `RUT9M_R_GPL_00.07.24.3/scripts/dockerbuild`.
...
2. Extract the  archive to an empty folder
   ...
   ```sh
   mkdir RUT9M_R_GPL_00.07.24.3
   tar -xzf ~/Downloads/RUT9M_R_GPL_00.07.24.3.tar.gz -C RUT9M_R_GPL_00.07.24.3
   cd RUT9M_R_GPL_00.07.24.3
   ```
...
5. Prepare packages for offline installation (optional)
   ...
   After this you can find packages in "RUT9M_R_GPL_00.07.24.3/bin/packages/<arch_name>/zipped_packages".
...
After successful build you will get the firmware file in `RUT9M_R_GPL_00.07.24.3/bin/targets/ramips/generic/tltFws`.
...
```text
RUT9M_R_GPL_00.07.24.3/package/feeds/vuci/vuci-ui-core/bin/dist
```
...
2. Generate banner text:
   ...
   ```sh
   figlet YOUR_TEXT > "RUT9M_R_GPL_00.07.24.3/package/base-files/files/etc/banner.logo"
   ```
...
1. Open the device family file:
   ...
   ```text
   RUT9M_R_GPL_00.07.24.3/target/linux/ramips/image/devices/rut9xx_family.mk
   ```
...
4. Regenerate tmpinfo files:
   ...
   ```sh
   rm -rf "RUT9M_R_GPL_00.07.24.3/tmp"
   ```
...
```text
RUT9M_R_GPL_00.07.24.3/package/base-files/files/lib/preinit/84_set_password
```
