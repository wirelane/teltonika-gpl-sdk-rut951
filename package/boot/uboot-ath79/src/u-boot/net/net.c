...
int NetLoopHttpd(void){
	...
	int httpd_wait_msec = 0;
	int httpd_timeout_enabled;
	int eth_init_max_tries;
	int eth_init_attempt = 0;
	...
	eth_init_max_tries = simple_strtol(getenv("eth_init_max_tries"), NULL, 10);
	while ((!eth_init_max_tries || eth_init_attempt < eth_init_max_tries) && !eth_init(bd)) {
		eth_init_attempt++;
		eth_halt();
		milisecdelay(1000);
	}

	if (eth_init_max_tries && eth_init_attempt == eth_init_max_tries) {
		eth_halt();
	...
	httpd_timeout_enabled = !strcmp(getenv("httpd_timeout_enabled"), "1");

	// infinite loop
	for(;;){
		if (!strcmp(getenv("httpd_finished"), "1")) {
			printf("Httpd execution finished, continuing boot...\n");
			return -1;
		}

		if (httpd_timeout_enabled) {
			milisecdelay(HTTPD_TIMEOUT_DELAY_MSEC);
			httpd_wait_msec += HTTPD_TIMEOUT_DELAY_MSEC;
			if (httpd_wait_msec >= HTTPD_TIMEOUT_MSEC) {
				printf("Httpd timed out, continuing boot\n");
			}
		}

		if ((httpd_timeout_enabled && httpd_wait_msec >= HTTPD_TIMEOUT_MSEC)) {
			eth_halt();
		...
		// Blink leds to indicate failsafe mode
		if (led_count++ > 1000) {
			if (led_on) {
				all_led_on();
#if defined(CONFIG_SHIFT_REG)
