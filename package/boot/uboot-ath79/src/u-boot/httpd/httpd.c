...
#define STATE_SERIAL_REQUEST	3		// remote host sent SERIAL request
...
extern const struct fsdata_file file_fail_checksum_html;
extern const struct fsdata_file file_fail_name_html;
...
static uint8_t serial_request_magic[] = {
	0x52, 0x45, 0x51, 0x55, 0x45, 0x53, 0x54, 0x20,
	0x53, 0x45, 0x52, 0x49, 0x41, 0x4c
};
	...
	if(start){
		...
		if(end){
			...
			if((end - (char *)uip_appdata) < uip_len){
				...
				// U-Boot
				if((webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_UBOOT) && (hs->upload_total > CONFIG_MAX_UBOOT_SIZE + 11)){
...
void httpd_appcall(void){
	...
	unsigned int i;
	ulong crc = 0, crc_current = 0;
	char crc_string[11] = "\0";
		...
		case HTONS(80):
			...
			if(uip_newdata() && hs->state == STATE_NONE){
				...
				if(uip_appdata[0] == ISO_G && uip_appdata[1] == ISO_E && uip_appdata[2] == ISO_T && (uip_appdata[3] == ISO_space || uip_appdata[3] == ISO_tab)){
					...
					size_t len = uip_len > sizeof(serial_request_magic) ?
						uip_len : sizeof(serial_request_magic);

					if (!memcmp((u8_t *)uip_appdata, serial_request_magic, len)) {
						u8_t *addr = (u8_t *)(KSEG1ADDR(CFG_FLASH_BASE) + OFFSET_MNF_INFO + OFFSET_SERIAL_NUMBER);

						printf("Serial request magic packet received\n");
						uip_send(addr, SERIAL_NUMBER_LENGTH);
						hs->state = STATE_SERIAL_REQUEST;
						setenv("httpd_timeout_enabled", "0");
						return;
					}
				}
				...
				if(hs->state == STATE_FILE_REQUEST){
					...
					if(hs->upload_total < 10240){
						fs_open(file_fail_name_html.name, &fsfile);
			...
			if (hs->state == STATE_SERIAL_REQUEST) {
				httpd_state_reset();
				uip_close();
				setenv("httpd_finished", "1");
			}
				...
				if(hs->state == STATE_UPLOAD_REQUEST){
					...
					if(hs->upload >= (hs->upload_total + strlen(boundary_value) + 6)){
						if(webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_UBOOT){
							ptr = (char *)CONFIG_LOADADDR;
							hs->upload_total = hs->upload_total - 11;
							router_name[0] = '\0';
							strncpy(router_name, (const char *)(ptr + hs->upload_total - 10 - 12), 12);

							crc_string[0] = '\0';
							strncpy(crc_string, (const char *)(ptr + hs->upload_total), 10);
							crc_string[10] = '\0';

							crc_current = atoi(crc_string);
							crc = crc32(0, (const unsigned char *)CONFIG_LOADADDR, hs->upload_total);
						}
						...
						if(!webfailsafe_upload_failed){
							if((webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_UBOOT) && (crc != crc_current)) {
								webfailsafe_upload_failed = 1;
								fs_open(file_fail_checksum_html.name, &fsfile);
							} else if ((webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_UBOOT) && (strncmp(router_name, DEVICE_MODEL, sizeof(DEVICE_MODEL) - 1))) {
								webfailsafe_upload_failed = 1;
								fs_open(file_fail_name_html.name, &fsfile);
							} else if ((webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_FIRMWARE) && (fwtool_validate_manifest((void *)CONFIG_LOADADDR, hs->upload_total))) {
								webfailsafe_upload_failed = 1;
								fs_open(file_fail_name_html.name, &fsfile);
							} else {
