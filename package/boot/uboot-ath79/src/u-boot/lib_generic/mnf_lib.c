...
inline static char int_to_hexchar(unsigned val)
{
...
inline static void bin_to_hexstr(const char *input, char *result, int len)
{
	int i;
	for (i = 0; i < len; i++) {
...
inline static int hexchar_to_int(char c)
{
	const char *hex_chars = "0123456789ABCDEF";
...
inline static int hexstr_to_bin(const char *input, char *result, int len)
{
	for (int i = 0; i < len; i++) {
