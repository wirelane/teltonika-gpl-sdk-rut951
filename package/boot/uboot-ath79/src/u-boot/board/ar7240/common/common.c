...
#define ALIGN_SIZE	"8"
	...
	#define CONFIG_UBOOT_TYPE "VerifiedBoot Recovery"
#elif defined(CONFIG_TLT_VERIFIED_BOOT)
	#define CONFIG_UBOOT_TYPE "VerifiedBoot"
#endif
...
#if defined(CONFIG_TLT_VERIFIED_BOOT)
		/* U-Boot type */
	printf("%" ALIGN_SIZE "s %s\n",
	       "TYPE:", CONFIG_UBOOT_TYPE);
#endif
	puts("\n");
}
...
int digital_in_status(void)
{
	...
	return !(gpio & (CONFIG_GPIO_MASK_DIGITAL_IN));
#else
	return 0;
#endif
}
...
{
#if defined(CONFIG_QCA_LED_ANIMATION_MASK)
	const int array[] = {
		CONFIG_QCA_LED_ANIMATION_MASK
	};
