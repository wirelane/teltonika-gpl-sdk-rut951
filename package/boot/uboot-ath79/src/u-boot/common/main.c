...
/* Recovery mode before normal boot */
    ...
    defined(CONFIG_GPIO_RESET_BTN)
		...
		if (reset_button_status()) {
			printf("Press button for at least\n 5 s to enter web based recovery\n\n");
		}

		for (stop_boot = 1; stop_boot < 301; stop_boot++) {
			if (!reset_button_status()) {
				break;
			}
		...
		printf("\n\n");
		/* Run httpd if button is pressed for at least 5 seconds */
		/* Continue booting if button is pressed for longer than 10 seconds */
		if (stop_boot >= 50 && stop_boot < 301) {
			setenv("httpd_timeout_enabled", "0");
			setenv("eth_init_max_tries", "0");
			run_command("httpd", 0);
		}
	...
	#endif /* CONFIG_BTN_RECOVERY_SCRIPTLESS */
...
#endif // CONFIG_GPIO_MASK_DIGITAL_IN
