...
static int display_banner(void)
{
	...
	printf("\n***************************************"
	       "\n*     %s     *"
	       "\n*     Build: "
	       MK_STR(CONFIG_BUILD_DATE_UTC) " (%s)       *"
	       "\n***************************************\n\n",
	       version_string, CUSTOM_VERSION_TXT);
...
{
#ifdef CONFIG_EARLY_GPIO_INIT
	return early_gpio_init();
#else
	return 0;
#endif
}
...
void board_init_f(ulong bootflag)
{
	...
	/* Run anything if needed, before init sequence */
	pre_init();
